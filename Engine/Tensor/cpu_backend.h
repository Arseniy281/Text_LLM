...
private:
    static Tensor MatMul2DAccelerate(const Tensor& A, const Tensor& B);
    static Tensor MatMul3D2DAccelerate(const Tensor& A, const Tensor& B);
    static Tensor MatMulBatchedFallback(const Tensor& A, const Tensor& B);
public:
    ~CPUBackend() = default;
